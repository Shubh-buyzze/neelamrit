 ...
 *
 * Login methods:
 * 1. Truecaller 1-tap (mobile only — Truecaller app required)
 * 2. Email + Password (manual)
 *
 * After Truecaller success:
 * - New user  → /complete-profile  (fill naam, email etc.)
 * - Old user  → /  (direct home)
 *
 * PATH FIX: Status API path is /api/auth/truecaller/status  (not /api/truecaller/status)
 */
...
export default function LoginPage() {
  ...
  const handleTruecallerLogin = () => {
    ...
    // Start polling after 800ms (give Truecaller time to open)
    setTimeout(() => {
      setStatus("polling");
      ...
      pollRef.current = setInterval(async () => {
        try {
          ...
          console.log("[TC-Poll]", data.status, "| isNew:", data.is_new_user);
          ...
          if (data.status === "success") {
            ...
            // Ghost email is always phone@neelamrit.com — used for signInWithPassword
          ...
          // Single network error → keep polling (don't kill on blip)
        }
      }, 2000);
    }, 800);
  ...
  const handleEmailLogin = async () => {
    ...
    if (authErr) {
      ...
      // Email login users: check if profile_complete
        ...
        <div className="flex-1 p-8 lg:p-14 flex flex-col justify-center">
          ...
          {statusMsg && (
            <div className="mb-5 p-4 rounded-2xl bg-emerald-50 border border-emerald-100 text-emerald-800 text-xs font-semibold flex items-center gap-2">
              <span className="text-base">{isSuccess ? "✅" : "⏳"}</span> {statusMsg}
            </div>
          )}
          {errorMsg && (
