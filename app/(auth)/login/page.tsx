...
function generateNonce(): string {
  return Math.random().toString(36).slice(2, 12) + Date.now().toString(36) + Math.random().toString(36).slice(2, 8); 
}
...
export default function LoginPage() {
  ...
  const handleTruecallerLogin = () => {
    ...
    killTimerRef.current = setTimeout(() => { setError("Request timeout हो गया। दोबारा try करें।"); }, 90_000);
  };
  ...
  const startPolling = (nonce: string) => {
    pollTimerRef.current = setInterval(async () => {
      ...
      } catch (e: any) { console.error("[TC-Poll] fetch error:", e.message); }
    }, 2000); 
  };

  const handleEmailLogin = async () => {
    if (!email.trim() || !password.trim()) { setErrorMsg("Email और Password दोनों भरें"); return; }
    setStatus("loading"); setStatusMsg("Verifying..."); setErrorMsg("");

    const { error: authErr } = await supabase.auth.signInWithPassword({ email, password });
    if (authErr) { setStatus("error"); setErrorMsg(authErr.message); setStatusMsg(""); } 
    ...
    <div className="min-h-screen bg-[#fafaf9] flex items-center justify-center p-4 font-sans">
      <div className="relative w-full max-w-5xl mx-auto bg-white rounded-[2.5rem] shadow-xl overflow-hidden flex flex-col lg:flex-row border border-gray-100">
        ...
        <div className="flex-1 p-8 lg:p-14 flex flex-col justify-center">
          ...
          {isMobile && (
            <>
              <button onClick={handleTruecallerLogin} disabled={isLoading || isSuccess} className="w-full mb-5 flex items-center justify-center gap-3 bg-[#0087FF] hover:bg-[#006FD6] active:scale-95 text-white py-4 px-6 rounded-2xl text-sm font-black tracking-wide shadow-lg transition-all duration-200 disabled:opacity-70 disabled:cursor-not-allowed">
                {status === "polling" ? "Truecaller में Approve करें..." : status === "loading" ? "खुल रहा है..." : "Truecaller से 1-Click Login"}
              </button>
              <div className="flex items-center gap-3 mb-5"><div className="flex-1 h-px bg-gray-200" /><span className="text-[10px] font-bold uppercase text-gray-400 tracking-widest">या Email से</span><div className="flex-1 h-px bg-gray-200" /></div>
            </>
          )}

          <div className="space-y-4">
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email Address" disabled={isLoading || isSuccess} className="w-full px-5 py-4 bg-gray-50 border border-gray-200 rounded-2xl text-sm font-medium text-gray-900 placeholder-gray-400 outline-none focus:border-amber-400 focus:bg-white disabled:opacity-60" />
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Password" disabled={isLoading || isSuccess} onKeyDown={(e) => e.key === "Enter" && handleEmailLogin()} className="w-full px-5 py-4 bg-gray-50 border border-gray-200 rounded-2xl text-sm font-medium text-gray-900 placeholder-gray-400 outline-none focus:border-amber-400 focus:bg-white disabled:opacity-60" />

            <button onClick={handleEmailLogin} disabled={isLoading || isSuccess} className="w-full py-4 rounded-2xl text-xs font-black uppercase tracking-widest bg-amber-900 hover:bg-gray-900 text-white shadow-lg disabled:opacity-60">
          ...
          <div className="mt-8 pt-6 border-t border-gray-100 text-center text-sm font-medium text-gray-500">
            New here? <Link href="/signup" className="text-amber-900 font-black hover:underline ml-1">Account बनाएं</Link>
          </div>
        </div>
      </div>
    </div>
