...
export default function SignupPage() {
  ...
  // 🟢 1. TRUECALLER 1-CLICK SIGNUP / LOGIN (Mobile Only)
  const handleTruecallerAuth = async () => {
    ...
    // Start Polling API to check if Webhook received data
  ...
  async function handleSignup() {
    ...
    if (form.phone.length !== 10) {
      setError("Enter valid 10-digit phone number"); return;
    }
    if (!acceptedTerms) {
      setError("Please accept the Terms & Conditions and Privacy Policy."); return;
    }
    ...
    <div className="min-h-screen bg-[#fafaf9] flex items-center justify-center p-4 sm:p-6 lg:p-8 font-sans selection:bg-amber-200">
      ...
      <div className="relative w-full max-w-5xl mx-auto">
        <div className="bg-white rounded-[2.5rem] shadow-[0_32px_64px_-16px_rgba(0,0,0,0.06)] border border-gray-100 overflow-hidden backdrop-blur-sm">
          <div className="flex flex-col lg:flex-row">
            ...
            <div className="flex-1 p-8 sm:p-12 lg:p-14">
              ...
              {successMsg && <div className="mb-6 p-4 rounded-2xl bg-emerald-50 border border-emerald-100 text-[12px] font-bold text-emerald-800 flex items-center gap-2 animate-in fade-in">✓ {successMsg}</div>}
              {error && <div className="mb-6 p-4 rounded-2xl bg-red-50 border border-red-100 text-[12px] font-bold text-red-800 flex items-center gap-2 animate-in shake duration-300">⚠️ {error}</div>}

              {/* 🟢 TRUECALLER 1-CLICK BUTTON (Visible only on mobile) */}
              <button
              ...
              >
                {loading && isPolling ? (
                  ...
                  <>
                    <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24"><path d="M20.2 15.6c-.6-.6-1.5-.6-2.1 0l-2.4 2.4c-4.3-2.1-7.8-5.6-9.9-9.9l2.4-2.4c.6-.6.6-1.5 0-2.1l-4.5-4.5c-.6-.6-1.5-.6-2.1 0l-1.4 1.4C-.5 5.5.9 14.2 8.7 21.9c7.7 7.7 16.4 9.2 21.3 3.8l1.4-1.4c.6-.6.6-1.5 0-2.1l-4.5-4.5z" /></svg>
              ...
              <div className="md:hidden flex items-center gap-4 mb-6 opacity-40">
                <div className="flex-1 h-px bg-gray-400"></div><span className="text-[10px] font-black uppercase tracking-widest text-gray-600">OR MANUAL SIGNUP</span><div className="flex-1 h-px bg-gray-400"></div>
              </div>
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <input type="text" placeholder="e.g. Shubham Dwivedi" value={form.full_name} onChange={(e) => setForm({ ...form, full_name: e.target.value })} className="w-full px-5 py-4 bg-gray-50 border border-gray-100 focus:border-amber-400 focus:bg-white rounded-2xl text-sm font-semibold text-gray-900 outline-none transition-all placeholder:text-gray-400" />
                </div>
                ...
                <div>
                  ...
                  <div className="flex bg-gray-50 border border-gray-100 focus-within:border-amber-400 focus-within:bg-white rounded-2xl overflow-hidden transition-all">
                    ...
                    <input type="tel" placeholder="9876543210" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value.replace(/\D/g, "").slice(0, 10) })} className="w-full px-4 py-4 bg-transparent text-sm font-semibold text-gray-900 outline-none placeholder:text-gray-400" />
                  </div>
                </div>

                <div>
                  <label className="block text-[11px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Email Address *</label>
                  <input type="email" placeholder="you@example.com" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className="w-full px-5 py-4 bg-gray-50 border border-gray-100 focus:border-amber-400 focus:bg-white rounded-2xl text-sm font-semibold text-gray-900 outline-none transition-all placeholder:text-gray-400" />
                </div>

                <div>
                  <label className="block text-[11px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Password *</label>
                  <input type="password" placeholder="Min. 6 characters" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} className="w-full px-5 py-4 bg-gray-50 border border-gray-100 focus:border-amber-400 focus:bg-white rounded-2xl text-sm font-semibold text-gray-900 outline-none transition-all placeholder:text-gray-400" />
                </div>

                <div className="flex items-start gap-3 pt-2">
                  <input type="checkbox" checked={acceptedTerms} onChange={(e) => setAcceptedTerms(e.target.checked)} className="w-4 h-4 text-amber-900 bg-gray-50 border border-gray-300 rounded cursor-pointer mt-0.5" />
                  <label className="text-[12px] font-medium text-gray-500 cursor-pointer">I agree to the Terms & Conditions and Privacy Policy.</label>
                </div>

                <button onClick={handleSignup} disabled={loading} className="w-full py-5 rounded-2xl text-[11px] font-black uppercase tracking-[0.25em] bg-amber-900 hover:bg-black text-white shadow-xl active:scale-95 mt-4 transition-all disabled:bg-gray-200 disabled:text-gray-400 disabled:shadow-none">
                  {loading && !isPolling ? "Creating Account..." : "Create Account"}
                </button>
              </div>

              <div className="mt-8 pt-6 border-t border-gray-50 text-center">
                <p className="text-sm font-medium text-gray-500">Already have an account? <Link href="/login" className="text-amber-900 font-black hover:underline">Sign In</Link></p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-10 flex flex-col items-center gap-3 opacity-60 hover:opacity-100 transition-opacity duration-300">
          <div className="flex items-center gap-2">
            <svg className="w-4 h-4 text-amber-800" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M2.166 4.999A11.954 11.954 0 0010 1.944 11.954 11.954 0 0017.834 5c.11.65.166 1.32.166 2.001 0 5.225-3.34 9.67-8 11.317C5.34 16.67 2 12.225 2 7c0-.682.057-1.35.166-2.001zm11.541 3.708a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" /></svg>
