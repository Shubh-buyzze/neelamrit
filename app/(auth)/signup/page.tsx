...
export default function SignupPage() {
  ...
  const handleTruecallerAuth = () => {
    ...
    // Note: The browser will be redirected away from this page by Truecaller
  };
      ...
      <div className="relative w-full max-w-5xl mx-auto">
        <div className="bg-white rounded-[2.5rem] shadow-xl border border-gray-100 overflow-hidden">
          <div className="flex flex-col lg:flex-row">
            ...
            <div className="flex-1 bg-gradient-to-br from-amber-50/50 to-white p-8 lg:p-12 flex flex-col items-center justify-center border-r">
            ...
            <div className="flex-1 p-8 sm:p-12 lg:p-14">
              ...
              <div className="md:hidden flex items-center gap-4 mb-6 opacity-30">
                <div className="flex-1 h-px bg-gray-400"></div><span className="text-[10px] font-black uppercase tracking-widest text-gray-600">OR MANUAL</span><div className="flex-1 h-px bg-gray-400"></div>
              </div>

              <div className="space-y-5">
                <input type="text" placeholder="Full Name" value={form.full_name} onChange={(e) => setForm({ ...form, full_name: e.target.value })} className="w-full px-5 py-4 bg-gray-50 border border-gray-200 focus:border-amber-400 focus:bg-white rounded-2xl text-sm font-semibold text-gray-900 outline-none transition-all" />
                <input type="tel" placeholder="Phone (10-digit)" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value.replace(/\D/g, "").slice(0,10) })} className="w-full px-5 py-4 bg-gray-50 border border-gray-200 focus:border-amber-400 focus:bg-white rounded-2xl text-sm font-semibold text-gray-900 outline-none transition-all" />
                <input type="email" placeholder="Email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className="w-full px-5 py-4 bg-gray-50 border border-gray-200 focus:border-amber-400 focus:bg-white rounded-2xl text-sm font-semibold text-gray-900 outline-none transition-all" />
                <input type="password" placeholder="Password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} className="w-full px-5 py-4 bg-gray-50 border border-gray-200 focus:border-amber-400 focus:bg-white rounded-2xl text-sm font-semibold text-gray-900 outline-none transition-all" />
                ...
                <button onClick={handleSignup} disabled={loading} className="w-full py-5 rounded-2xl text-[11px] font-black uppercase tracking-widest bg-amber-900 text-white shadow-xl hover:bg-black transition-all">
